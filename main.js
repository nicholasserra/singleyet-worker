...
singleYet = function(){
    ...
    client.query('SELECT user_id, followed.fb_id, rel_status, access_token, email, followed.id AS followed_id FROM `followed` JOIN `user` on followed.user_id = user.id', function iterate(error, results) {
